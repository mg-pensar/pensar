// author : Mauricio Gomes

#include <catch2/catch_test_macros.hpp>

#include "../array.hpp"
#include "../data.hpp"
#include "../cs.hpp"
#include "../s.hpp"
#include "../io_util.hpp"
#include "../file.hpp"
#include "../equal.hpp"

namespace pensar_digital::cpplib
{
    const std::byte ZERO = std::byte{0};
    using DummyPadding = std::array<std::byte, 7>;
    constexpr DummyPadding empty_padding = {ZERO,ZERO,ZERO,ZERO,ZERO,ZERO,ZERO};
    struct ArrayTestDummy
    {
        int64_t i;          // 8-aligned
        CS<0, 16> cs10;     // make CS size a multiple of 8 (change to 16)
        char c;
        DummyPadding pad;   // explicit tail padding

        ArrayTestDummy(int64_t i_ = 0, const C* str = W(""), char c_ = '\0', DummyPadding p = empty_padding) noexcept
            : i(i_), cs10(str), c(c_), pad(p)
        {
        }

        bool operator==(const ArrayTestDummy& other) const noexcept
        {
            return equal<ArrayTestDummy>(*this, other);
        }
    };

    static_assert(StdLayoutTriviallyCopyable<ArrayTestDummy>);

    TEST_CASE("CArray", "[array]")
    {
        using Dummy = ArrayTestDummy;
        using DummyArray = CArray<3, Dummy>;
        static_assert(StdLayoutTriviallyCopyable<DummyArray>);

        const Dummy d0 = { 0, W("blah"), 'a' };
        const Dummy d1 = { 1, W("bléh"), 'b' };
        const Dummy d2 = { 2, W("blih"), 'c' };
        CArray<3, Dummy> a = {d0, d1, d2};
        INFO("size must be 3"); CHECK(a.size() == 3);

        DummyArray a2 = a; // Copy constructor
        INFO("arrays must match"); CHECK(a == a2);

        DummyArray a3 = {d0, d1};
        INFO("arrays must not match"); CHECK(a != a3);

        CHECK(a.compare_range(a2, 3));
        CHECK(a.compare_range(a2, 0, 2));
        CHECK(!a.compare_range(a2, 1, 3));

        DummyArray d_array = {d0, d1, d2};
        CHECK(a.compare_range(d_array, 3));

        auto res = a.contains(d1);
        CHECK(res);
        CHECK(res.mresult == 1);
        res = a.contains(Dummy{0, W("notfound"), 'x'});
        CHECK(!res);

        std::array<Dummy, 3> std_array = {d0, d1, d2};
        CHECK(a.compare_range(std_array.data(), 3));
        static_assert(StdLayoutTriviallyCopyable<std::array<Dummy,3>>);
    }
}
