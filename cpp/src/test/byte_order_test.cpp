...
namespace pensar_digital::cpplib
{
    ...
    void test_byte_order_conversion(std::vector<U> a, std::vector<T> expected, uint_fast8_t count = 0)
    {
        std::vector<T> a6;
        for (auto x : a)
            a6.push_back(x);
        ...
        if (sizeof(U) > sizeof(std::byte))
        {
            if ((sizeof(T) == sizeof(U)))
            {
                ...
                static_assert(IsLikeArrayOfV<pd::Array<T>, T>);
                static_assert(IsLikeArrayOfV<std::vector<T>, T>);

                INFO(pd::to_string<decltype(count)>(count++) + W(". forward conversion"));
                check_equal_collection(a3, expected);

                pd::convert<>(b, size, pd::big_address_8_byte_order, pd::native_byte_order);
                std::memcpy(c.data(), b.data(), b.size());
                std::vector<T> a5(c.data(), c.data() + c.size());

                INFO(pd::to_string<decltype(count)>(count++) + W(". reverse conversion"));
                check_equal_collection(a5, a6);
            }
        }
    }

    TEST_CASE("ByteOrder1", "[byte_order]")
    {
        std::vector<uint8_t> a1 = { 0, 1, 2, 3, 4, 5, 6, 7, 8 };
        std::vector<uint8_t> a2 = { 0, 1, 2, 3, 4, 5, 6, 7, 8 };
        test_byte_order_conversion<uint8_t>(a1, a2, 0);
        ...
        test_byte_order_conversion<uint16_t>(a3, expected, 1);
        ...
        test_byte_order_conversion<uint32_t>(a5, a6, 2);
        ...
        test_byte_order_conversion<int16_t, uint16_t>(a11, a12, 5);
        ...
        test_byte_order_conversion<int32_t, uint32_t>(a13, a14, 6);
        ...
        test_byte_order_conversion<int64_t, uint64_t>(a15, a16, 7);
    }
}
